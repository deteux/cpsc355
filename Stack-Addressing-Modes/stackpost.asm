//Using stack to store local variables using post-indexed addressing 
...
main:
	stp fp, lr, [sp, -16]!              //Save FP and LR to stack, pre-increment sp, allocating 16 bytes
	mov fp, sp
	mov y_reg, 200                  	//Y = 200
	mov x_reg, 80                   	//X = 80

	str y_reg, [fp], -4               	//Store the value inside y_reg at the address stored inside fp, post increment fp, allocate 4 bytes
	str x_reg, [fp], -4              	//Store x_reg at current address in fp and postincrement, allocate 4 bytes
...
exit:
	add fp, fp, 4
	ldr w1, [fp], 4             	    //Increment fp by 4 and Load x1 with value stored at fp
	ldr w2, [fp], 4                 	//Increment fp by 4 and Load x2 with value stored at fp
